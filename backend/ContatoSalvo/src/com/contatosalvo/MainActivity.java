...
public class MainActivity extends Activity {
	...
	private void detalheContato(String nome) {
		HttpClient client = createHttpClient();
		...
		try {
			...
			if (entity != null) {
				...
				JSONArray telefones = jsonObject.getJSONArray("telefones");
				for (int i = 0; i < telefones.length(); i++) {
					JSONObject t = telefones.getJSONObject(i);
					String numero = t.optString("numero");
					//String operadora = t.optString("operadora");
					numeros.add(numero);
				}

//				JSONArray emailsJson = contato.getJSONArray(2).getJSONArray(1);
//				for (int i = 0; i < emailsJson.length(); i++) {
//					emails.add((String) emailsJson.get(i));
//				}
				...
				ArrayAdapter<String> listAdapterTelefone = new ArrayAdapter<String>(this, android.R.layout.simple_list_item_1, numeros);
				mainListViewTelefone.setOnItemClickListener(detalheContatoListener(mainListViewTelefone));
				mainListViewTelefone.setAdapter(listAdapterTelefone);
